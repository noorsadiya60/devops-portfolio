...
Definition of Done
 ...
 EC2 + SG created and destroyed via Terraform — zero manual clicks
 Remote state verified in S3 with DynamoDB locking configured
 State migrated from local to S3 via terraform init -migrate-state
 Drift introduced manually, detected by plan, reconciled by apply
